  ...
  <div class="module-container">
    <div v-if="showIndexFlag">
      ...
      <div class="filter-container">
              <module-select v-model="params.inventoryId" placeholder="请选择库存ID" class="filter-item" titleName="drugId" url="/inventory/loadAll" optionKey="id"></module-select>
        <el-input v-model="params.customerName" placeholder="请输入客户名称" class="filter-item" clearable />
      ...
      <el-table :data="dataList" style="width: 100%" @selection-change="handleSelectionChange" border stripe>
        <el-table-column type="selection" width="55" header-align="center" align="center" />
                <el-table-column prop="inventoryId" label="库存ID" header-align="center" align="center">
                  <template #default="{ row }" >
                    {{row.t2Drugid}}
                  </template>
                </el-table-column>
              <el-table-column prop="salesQuantity" label="销售数量" header-align="center" align="center"></el-table-column>
              <el-table-column prop="salesTime" label="销售时间" header-align="center" align="center"></el-table-column>
...
// 获取数据列表
const getDataList = () => {
  request.post("salesRecord/page", params.value).then(({ data }) => {
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm("确定要删除选中的信息吗?", "提示", {
    ...
    .then(() => {
      ...
      request.post("salesRecord/del/batch", ids).then(() => {
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确定要删除该信息吗?`, "提示", {
    ...
    .then(() => {
      request.delete(`salesRecord/del/` + row.id).then(() => {
        ElMessage.success("删除成功");
        getDataList();
      });
    })
