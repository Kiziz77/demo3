...
<template>
    <div class="theme-detail-container">
        ...
        <h1 class="detail-title">详情页</h1>
                        <!-- 图片轮播 -->
                <el-carousel v-if="detailData.drug_image" height="600px">
                    <el-carousel-item v-for="(img, index) in detailData.drug_image?.split(',')" :key="index">
                        <img :src="img" class="carousel-image" alt="详情图片" />
                    </el-carousel-item>
                </el-carousel>
        ...
        <div class="info-section">
                        <div class="info-item">
                            ...
                            <span class="value">{{detailData.drugId}}</span>
                        </div>
                        <div class="info-item">
                            <span class="label">单价:</span>
                            <span class="value">{{detailData.unitPrice}}</span>
                        </div>
...
<script setup>
    ...
    const tableName = "drug_display";
    const commentTypeId = ref("942209");
    const fieldNames = ref([]);
    // Tab状态
    const activeTab = ref("detail");

    const loadDetailData = ()=>{
        request.get("/drugDisplay/info/" + route.query.id).then(({data})=>{
            detailData.value = data;
        })
    }

    onMounted(() => {
        fieldNames.value[1] = "drug_id";
        fieldNames.value[0] = "drug_image";
