<template>
  <div class="module-showcase">
    <!-- 标题部分 -->
    <div class="header">
      <h1 class="title">药品售卖</h1>
    </div>

    <!-- 搜索条件 -->
    <div class="search-area">
      <el-input
          v-model="searchParams.productsName"
          placeholder="请输入药品名称"
          class="search-input"
          clearable
          @clear="handleSearch"
        ...
        <el-col v-for="item in dataList" :key="item.id" :xs="24" :sm="12" :md="8" :lg="6" :xl="4">
          <el-card class="module-card" shadow="hover" @click="handleModuleClick(item.id)">
            <div class="card-image">
              <img :src="item.imageUrl?.split(',')[0]" :alt="item.imageUrl"/>
            </div>
            <div class="card-content">
              <h3 class="module-title" v-if="item.productsName">{{ item.productsName }}</h3>
...
const loadDataList = async () => {
  const {data} = await request.post("/products/list", searchParams);
...
const handleModuleClick = (id) => {
  router.push({name: "productsDetail", query: {id: id}});
};
